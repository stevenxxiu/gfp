...
class ResultsData extends NodeData {
  *getChildren() {
    for (const child of this.node.querySelectorAll('.gsc-table-result')) yield new TextData(child)
  }
}
...
export default function (searchGui, config) {
  if (window.location.href.indexOf('/cse?') == -1 && window.location.href.indexOf('/custom?') == -1) return searchGui
  const mainNode = document.getElementById('cse')
  if (!mainNode) return searchGui
