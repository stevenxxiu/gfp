...
export class MultiRegExpFilter extends ActiveFilter_ {
  constructor(text, filters) {
    ...
    for (const filter of filters) filter.parent = this
  }

  get collapse() {
    for (const filter of this.filters) if (filter.collapse) return true
    return false
  }
  ...
  static fromText(text) {
    ...
    for (let i = 0; i < parts.length; i += 2) {
      ...
      if (filter instanceof InvalidFilter) return new InvalidFilter(text)
...
Filter.fromObject = function (text, obj) {
  ...
  if (res instanceof ActiveFilter) {
    ...
    if ('lastHit' in obj) res._lastHit = parseInt(obj.lastHit) || null
  }
  return res
}
